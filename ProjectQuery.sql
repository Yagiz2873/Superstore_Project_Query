...
USS.Category, USS.ShipMode, USS.Sub_Category, USS.Country, USS.CustomerName, USS.[State] from UpdatednewSuperstore USS LEFT JOIN  SuperstoreRFM SRFM On SRFM.CustomerID = USS.CustomerID
*/
...
FROM UpdatednewSuperstore USS LEFT JOIN  SuperstoreRFM SRFM ON SRFM.CustomerID  = USS.CustomerID
...
/*Müşteri ilk-son sipariş*/ 
Select USS.CustomerID, MIN(OrderDate) FirstOrderDate ,MAX(OrderDate) LastOrderDate, DATEDIFF(DAY,MIN(OrderDate),MAX(OrderDate)) CustomerOrderDayDiff,
AVG(SRFM.Tenure) AvgTenure, AVG(SRFM.TenureInMonths) AvgTenureMonth  ,AVG(SRFM.Recency) AvgRecency,  AVG(SRFM.Frequency) AvgFrequency, AVG(USS.Sales) AvgSales , SUM(USS.Sales) TotalSales ,AVG(SRFM.Monetary) AvgMonetary, SUM(SRFM.Monetary) TotalMonetary,
AVG(USS.Revenue) AvgRevenue, SUM(USS.Revenue) TotalRevenue ,AVG(SRFM.RecencyScore) RS , AVG(SRFM.FrequencyScore) FS, AVG(SRFM.MonetaryScore) MS, AVG(SRFM.BasketSize) AvgBasketSize, SUM(SRFM.BasketSize) TotalBasketSize
from UpdatednewSuperstore USS LEFT JOIN SuperstoreRFM SRFM ON USS.CustomerID = SRFM.CustomerID
Group by USS.CustomerID 



/* En fazla alışveriş yapan müşteri */
Select USS.CustomerID, COUNT(USS.OrderID)  ,MIN(OrderDate) FirstOrderDate ,MAX(OrderDate) LastOrderDate, DATEDIFF(DAY,MIN(OrderDate),MAX(OrderDate)) CustomerOrderDayDiff,
AVG(SRFM.Tenure) AvgTenure, AVG(SRFM.TenureInMonths) AvgTenureMonth  ,AVG(SRFM.Recency) AvgRecency,  AVG(SRFM.Frequency) AvgFrequency, AVG(USS.Sales) AvgSales , SUM(USS.Sales) TotalSales ,AVG(SRFM.Monetary) AvgMonetary, SUM(SRFM.Monetary) TotalMonetary,
AVG(USS.Revenue) AvgRevenue, SUM(USS.Revenue) TotalRevenue ,AVG(SRFM.RecencyScore) RS , AVG(SRFM.FrequencyScore) FS, AVG(SRFM.MonetaryScore) MS, AVG(SRFM.BasketSize) AvgBasketSize, SUM(SRFM.BasketSize) TotalBasketSize
from UpdatednewSuperstore USS LEFT JOIN SuperstoreRFM SRFM ON USS.CustomerID = SRFM.CustomerID
Group by USS.CustomerID 
ORDER BY COUNT(USS.OrderID) DESC


/*SuperstoreDetailedCustomer*/
select * from  UpdatednewSuperstore  USS LEFT JOIN SuperstoreRFM SRFM ON USS.CustomerID = SRFM.CustomerID   
